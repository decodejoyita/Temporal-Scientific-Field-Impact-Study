#Carries out the temporal Study 2 years.
...
for fl in range(len(arr)):
    ...
    while year<=years[1]:
        ...
        if fos in fosid_pid:
            ...
            for p in papers:
                if (p in pid_year and (pid_year[p]==(year-1) or pid_year[p]==(year-2))):
                    pubslist.append(p)
    ...
    idd=str(fos)
    #print(idd)
    with open('/home/saswata/FYP/Dataset/Fos/Temporal(2years)/'+idd+'.csv','wb') as f:
        writer = csv.writer(f)
